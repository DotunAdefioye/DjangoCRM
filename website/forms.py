...
class SignUpForm(UserCreationForm):
    user_name = forms.CharField(label="Username", max_length=100, unique=True, widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'User Name'}))
    email = forms.EmailField(label="Email", unique=True, widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Email Address'}))
    ...
    phone_number = forms.PhoneNumberField(label="Phone Number", unique=True, max_length=20, widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Phone Number'}))
    date_of_birth = forms.DateField(('Date of birth'), null=True)
